...
def createOneHotDict(inputData):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    di = (inputData.flatMap(lambda row: row).distinct())
    di2=(di.zipWithIndex().collectAsMap())
    return di2
...
def parsePoint(point):
    ...
    """
    features1 = point.split(',')[1:]
    return [(idx, value) for (idx, value) in enumerate(features1)]
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    for (featureID, value) in rawFeats:
        try:
            xx.append((OHEDict[(featureID, value)],1))
        except KeyError:
             pass
    return SparseVector(numOHEFeats, xx)
...
def evaluateResults(model, data):
    ...
    """
    return (data
            .map(lambda xx: (xx.label, getP(xx.features, model.weights, model.intercept)))
...
# TODO: Replace <FILL IN> with appropriate code
logLossValBase = OHEValidationData.map(lambda xx: computeLogLoss(classOneFracTrain, xx.label)).mean()
...
def computeSparsity(data, d, n):
    ...
    """
    y=data.map(lambda xx: len(xx.features.indices)).mean()
    return (y/d)
...
OHETestData.cache()
logLossTestBaseline = OHETestData.map(lambda xx: computeLogLoss(classOneFracTrain, xx.label)).mean()
