...
print numPoints
samplePoints = rawData.cache().take(5)
print samplePoints
...
def calcRMSE(labelsAndPreds):
    ...
    """
    nn=labelsAndPreds.count()
    sumall=labelsAndPreds.map(lambda x:squaredError(x[0],x[1])).reduce(lambda x,y:x+y)
    return (sumall/nn)**0.5
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda trainData: getLabeledPrediction(w,trainData))
...
def twoWayInteractions(lp):
    ...
    """
    s1 = list(itertools.product(range(len(lp.features)),range(len(lp.features))))
    return LabeledPoint(lp.label, np.hstack((lp.features,[lp.features[x] * lp.features[y] for (x,y) in s1])))
